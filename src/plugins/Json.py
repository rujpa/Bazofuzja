...
class Json(FileConnection.FileConnection):
    
    def getFileAsDataFrame(self):
        
        try:
            table = pd.read_json(self.filepath, encoding = 'unicode_escape')
    ...
    def exportDataFrameToFile(self, data):
        try:
            data.getDataFrame().to_json(self.setMultipleFilenames(data))
